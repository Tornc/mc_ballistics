...
RADAR = Radar(TARGET_POS, 250, 1, 0)
...
vel = Vector(1, 2, 3)
vms = vel.length() * 20
y, p = velocity_to_angles(vel)
traj = simulate_trajectory(Cannon(pos, vms, 0, g, cd, y, p, 0, 0), t)
print(f"Position: {traj[-1][1].round(1e-6)}")


fp, fv = forward_state(pos, vel, cd, g, t)
print()
print(f"Position: {fp}")
print(f"Velocity: {fv}")

ip, iv = inverse_state(fp, fv, cd, g, t)
print()
print(f"Position: {ip.round(1e-6)}")
print(f"Velocity: {iv.round(1e-6)}")
