...
with st.form("targets_form"):
    column_sizes = [0.2, 0.25, 0.25, 0.25, 0.2]
    hcol = st.columns(column_sizes)
    hcol[1].markdown(
        '<div class="centered-text">X</div>', unsafe_allow_html=True
    )
    hcol[2].markdown(
        '<div class="centered-text">Y</div>', unsafe_allow_html=True
    )
    hcol[3].markdown(
        '<div class="centered-text">Z</div>', unsafe_allow_html=True
    )
    for i, target in enumerate(st.session_state["targets"]):
        icol = st.columns(column_sizes)
        icol[0].markdown(
            f'<div class="centered-text">{f"#{i}"}</div>',
            unsafe_allow_html=True,
        )
        x = icol[1].number_input(
            "", value=target.x, key=f"x_{i}", label_visibility="collapsed"
        )
        y = icol[2].number_input(
            "", value=target.y, key=f"y_{i}", label_visibility="collapsed"
        )
        z = icol[3].number_input(
            "", value=target.z, key=f"z_{i}", label_visibility="collapsed"
        )
        remove = icol[4].form_submit_button(
            "❌", key=f"remove_{i}", help="Remove target."
        )
    b_col = st.columns(column_sizes)
    add_new = b_col[0].form_submit_button("➕", help="Add new target.")
    submitted = b_col[1].form_submit_button("💾", help="Submit changes.")